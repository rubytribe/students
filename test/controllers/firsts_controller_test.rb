...
class FirstsControllerTest < ActionController::TestCase
  ...
  test "should create first" do
    assert_difference('First.count') do
      post :create, first: { Last: @first.Last, birthday: @first.birthday, name: @first.name, name: @first.name }
    end
  ...
  test "should update first" do
    patch :update, id: @first, first: { Last: @first.Last, birthday: @first.birthday, name: @first.name, name: @first.name }
    assert_redirected_to first_path(assigns(:first))
  end
